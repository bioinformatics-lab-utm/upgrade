process CONCOCT {
    ...
    // FIXED: Use biocontainers image instead of installing at runtime
    // This ensures reproducibility and faster startup
    container 'quay.io/biocontainers/concoct:1.1.0--py39hf95cd2a_1'
    ...
    echo "Using CONCOCT from biocontainers image" | tee -a ${sample_id}.concoct.log
    ...
    # Step 2: Generate coverage table from BAM
    ...
    if command -v jgi_summarize_bam_contig_depths &> /dev/null; then
        ...
        samtools depth -aa ${bam} | awk '{sum[\$1]+=\$3; count[\$1]++} END {for(contig in sum) print contig"\\t"sum[contig]/count[contig]}' > ${sample_id}_concoct_work/contig_depths.txt
            ...
            LC_ALL=C sort ${sample_id}_concoct_work/contig_lengths.txt > ${sample_id}_concoct_work/contig_lengths.sorted.txt
            LC_ALL=C sort ${sample_id}_concoct_work/contig_depths.txt > ${sample_id}_concoct_work/contig_depths.sorted.txt
            join -t \$'\\t' ${sample_id}_concoct_work/contig_lengths.sorted.txt ${sample_id}_concoct_work/contig_depths.sorted.txt | \\
    ...
    if [ \$CONCOCT_EXIT -ne 0 ]; then
        echo "ERROR: CONCOCT clustering failed with exit code \$CONCOCT_EXIT" | tee -a ${sample_id}.concoct.log
        exit \$CONCOCT_EXIT
    fi
    ...
    if [ -f "${sample_id}_concoct_work/clustering_gt${min_contig_length}.csv" ]; then
        ...
        echo "WARNING: No clustering file found" | tee -a ${sample_id}.concoct.log
    fi

    # Step 5: Extract bins in FASTA format
    ...
    if [ -s "${sample_id}_concoct_clustering_gt${min_contig_length}.csv" ] && [ \$(wc -l < ${sample_id}_concoct_clustering_gt${min_contig_length}.csv) -gt 1 ]; then
        ...
        echo "WARNING: No valid clusters found, no bins to extract" | tee -a ${sample_id}.concoct.log
    fi
