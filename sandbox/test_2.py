...
class EnhancedWeatherProducerCSV:
    ...
    async def fetch_comprehensive_weather_data(self, session: aiohttp.ClientSession, city: Dict) -> Dict:
        """Fetch comprehensive weather data including all available parameters"""
        ...
        # Reduced hourly parameters for better API reliability
        hourly_params = [
        ...
        params = {
            ...
            'past_days': 1,  # Include yesterday for trends
            'forecast_days': 3  # Include short-term forecast
        }
        
        try:
            async with session.get(self.open_meteo_url, params=params, timeout=60) as response:
                if response.status == 200:
                    ...
                    logger.info(f"Successfully collected comprehensive data for {city['name']}, {city['county']} "
            ...
            logger.error(f"Error fetching data for {city['name']}: {str(e)}")
