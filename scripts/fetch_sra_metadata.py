...
def main():
    parser = argparse.ArgumentParser(description='Fetch metagenomic IDs with extended metadata - ONT focused')
    ...
    parser.add_argument('--platform', default='OXFORD_NANOPORE', choices=['OXFORD_NANOPORE', 'ILLUMINA', 'ANY'], help='Sequencing platform (default: OXFORD_NANOPORE)')
    parser.add_argument('--require-geo', action='store_true', help='Require geographic information (lat/lon or geo_loc_name)')
    ...
    if args.require_geo:
        print("Requiring geographic information")
    print("=" * 80)
    
    # Build query
    parts = ['metagenome[Organism]']
    
    # Add platform filter to query (improves search efficiency)
    if args.platform == 'OXFORD_NANOPORE':
        parts.append('Oxford Nanopore[Platform]')
    elif args.platform == 'ILLUMINA':
        parts.append('Illumina[Platform]')
    ...
    if args.platform != 'ANY':
        samples = [s for s in samples if s['platform'] == args.platform]
        print(f"✓ Filtered: {len(samples)} samples with {args.platform} platform")
    ...
    if args.require_geo:
        # Accept samples with either numeric coords OR geo_loc_name
        samples = [s for s in samples if 
                   (s['lat'] != 'N/A' and s['lon'] != 'N/A') or 
                   (s['geo_loc_name'] != 'N/A' and s['geo_loc_name'].strip())]
        print(f"✓ Filtered: {len(samples)} samples with geographic information")
