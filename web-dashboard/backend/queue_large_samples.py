...
async def main():
    ...
    if not minio.bucket_exists('genomic-bronze'):
        ...
        logger.info("Created genomic-bronze bucket")
    ...
    for sample_code, meta in SAMPLE_METADATA.items():
        ...
        if not os.path.exists(fastq_path):
            logger.warning(f"SKIP {sample_code}: file not found at {fastq_path}")
            continue

        file_size = os.path.getsize(fastq_path)
        logger.info(f"[{sample_code}] {file_size/1e9:.2f}GB - uploading to Bronze...")
        ...
        logger.info(f"Uploaded to Bronze: {object_key}")
        ...
        if not sample_id:
            ...
            logger.info(f"Created sample record (id={sample_id})")
        else:
            logger.info(f"Sample exists (id={sample_id})")
        ...
        queued += 1
        logger.info(f"Queued (pipeline_id={pipeline_id}, job={job_id[:16]}...)")

    await conn.close()
    logger.info(f"Done: {queued}/{len(SAMPLE_METADATA)} samples queued")
